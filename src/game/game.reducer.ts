...
export class GameReducer {
  ...
  yaniv(gameState: GameState): GameState {
    ...
    const winnerByScore = this.getMinScoreWinners(otherPlayers, currentPlayer);
    const asaf = winnerByScore.id !== currentPlayer.id;
    const playersRoundScores = this.calculateRoundScores(newState, currentPlayer, asaf);
    const winner = this.getRoundResultWinner(playersRoundScores);
  ...
  getRoundResultWinner(playersRoundScores: PlayerRoundScore[]): Player {
    const minDiffScore: number = playersRoundScores.map(score => score.diff).sort()[0];
    const winners: Player[] = playersRoundScores.filter(score => score.diff === minDiffScore).map(score => score.player);
    return this.getRandomItemFromArray(winners);
  }

  private calculateRoundScores(newState: GameState, currentPlayer: Player, asaf: boolean): PlayerRoundScore[] {
    return this.getActivePlayers(newState).map(player => {
  ...
  getMinScoreWinners(otherPlayers: Player[], currentPlayer: Player): Player {
    const sortedScores = otherPlayers.map(player => cardsScore(player.cards)).sort();
    ...
    return this.getRandomItemFromArray(winnerPlayers);
  }
